...
hosts["vidlox"] = function(resource, config, extraInfo, callback) {
	var headers = {
	...
	console.log("----------------- vidlox REQUEST -----------------", resource, config)
	libs.request_getHTML(resource.file, headers, resource).then(function(response) {
		...
		if (source && source.length > 0) {
			...
			source = `parse = ${source}`
			eval(source)
			console.log("------------ vidlox DATA HTML -----------", parse)
			for (var item of parse) {
				...
				libs.request_getHeader(item, 'HEAD', {}, {extraInfo: response.extraInfo, url: item}).then(function(res) {
					...
					if (fileSize > 0) {
						...
						res.extraInfo.extraInfo.host = "VIDLOX"

						console.log("-------------------- vidlox RESULT ----------------", fileSize, res.extraInfo)
