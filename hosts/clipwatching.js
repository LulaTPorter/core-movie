...
hosts["clipwatching"] = function(resource, config, extraInfo, callback) {
	var headers = {
	...
	console.log("----------------- clipwatching REQUEST -----------------", resource, config)
	libs.request_getHTML(resource.file, headers, resource).then(function(response) {
		...
		if (source && source.length > 0) {
			...
			console.log("------------ clipwatching DATA HTML -----------", parse)
			for (var item of parse) {
				...
				libs.request_getHeader(item, 'HEAD', {}, {extraInfo: response.extraInfo, url: item}).then(function(res) {
					...
					if (fileSize > 0) {
						...
						res.extraInfo.extraInfo.host = "CLIPWATCH"

						console.log("-------------------- clipwatching RESULT ----------------", fileSize, res.extraInfo)
		...
		console.log("--------- ERROR clipwatching------------",  e)
	})
};